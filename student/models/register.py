...
class StudentReg(models.Model):
    ...
    def action_print_booking(self):
        building_meterial = self.env.ref(
            'student.action_report_student_register')
        print('building_meterial', building_meterial)
        if building_meterial:
            report_id = self.env.ref(
                'student.action_report_student_register').id
            printer = self.env.ref(
                'student.action_report_student_register').default_print_option
            rep_name = self.env.ref(
                'student.action_report_student_register').report_name
            result = {'type': 'ir.actions.report', 'id': report_id,
                      'report_type': "qweb-pdf",
                      'binding_type': 'report',
                      'attachment': "",
                      'print_report_name': "(object._get_report_base_filename())",
                      'report_file': "report_student_register",
                      'report_name': rep_name,
                      'default_print_option': printer,
                      'multi': 'false',
                      'binding_view_types': "list,form",
                      'model': 'student.registration',
                      'xml_id': "student.action_report_student_register",
